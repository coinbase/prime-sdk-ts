 ...
 *
 * Usage:
 *   npm run example:ts examples/ts/orders/listPortfolioOrders.ts [orderStatuses] [productIds] [orderType] [orderSide] [startDate] [endDate]
 *
 * Examples:
 *   npm run example:ts examples/ts/orders/listPortfolioOrders.ts
 *   npm run example:ts examples/ts/orders/listPortfolioOrders.ts FILLED
 *   npm run example:ts examples/ts/orders/listPortfolioOrders.ts FILLED BTC-USD,ETH-USD
 *
...
const orderType = process.argv[4] as OrderType | undefined;
const orderSide = process.argv[5] as OrderSide | undefined;
const startDate = process.argv[6];
const endDate = process.argv[7];

async function listPortfolioOrdersExample(): Promise<void> {
  ...
  try {
    ...
    const request: ListPortfolioOrdersRequest = {
      portfolioId,
    };

    if (orderStatusesArg) {
      request.orderStatuses = orderStatusesArg.split(',') as OrderStatus[];
    }
